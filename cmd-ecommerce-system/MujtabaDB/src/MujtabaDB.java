...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 - USE OF '/' SLASH IS NOT ALLOWED IN DATABASE ENTERIES. IT CONFLICTS WITH
   ...
   if u must use slash, try writing SYMBOL_SLASH or something
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 !TODO's:
 - Wtdafak What is the use of ffforeign key? how can it be relational if no use of
  foreign key?
 - Return Data[] array if foreign key is accessed.
...
class Table {
    ...
    int _addRow(String[] data){
        if(tableLayout.hasIllegalCharacters(data)) return -1;
        if(!tableLayout.isGivenDataAccordingToTableLayout(data)) return -1;
        if(isGivenDataPKAlreadyExists(data)) return -1;
...
public class MujtabaDB {
    ...
    public void _load(String dbFilename) {
        ...
        try { scanner = new Scanner(new File(dbFilename)); } catch(Exception e){ System.out.println("DB Load Error"); }
